...
function ConfirmedPatients() {
	...
	const handleBillID = async (appointmentId) => {
		...
		try {
			...
			if (response.data && response.data.length > 0) {
				setExistingBill({ ...response.data[0], isNew: false });
				toast.success("Bill loaded successfully");
			} else {
				// 2. No bill exists → fetch prescription for treatments
				const presRes = await axios.get(
					`${VITE_REACT_APP_BASE_URL}/prescription?appointmentId=${appointmentId}`
				);

				if (presRes.data && presRes.data.length > 0) {
					const prescription = presRes.data[0];

					// Prefill bill items from treatments
					const treatmentItems = prescription.treatments?.map((t) => ({
						amount: 0, // you can calculate or let user enter
						description: t.treatment, // or map to human readable label
					}));
					...
					toast.success("Prefilled bill with treatments");
				} else {
					setExistingBill(null);
					toast("No bill or prescription found");
				}
			}
	...
	const handleBillSave = async (billData) => {
		try {
			// Show loading toast
			const loadingToast = toast.loading(
				exhistingBill && !exhistingBill.isNew
					? "Updating bill..."
					: "Creating bill..."
			);

			if (exhistingBill && !exhistingBill.isNew) {
