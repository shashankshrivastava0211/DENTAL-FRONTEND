...
const handleAiCall = async () => {
  console.log("AI Call Triggered", prescription.symtoms);
  setLoading(true); // show loader
  try {
    const response = await fetch("http://localhost:3000/api/v1/ml/olama-diagnosis", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        symptoms: prescription.symtoms,
      }),
    });
    ...
    // Update diagnosis field with response
    if (data?.diagnosis) {
      ...
      alert("AI did not return a diagnosis.");
    }
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div>
            ...
            <textarea
              value={prescription.symtoms}
              onChange={(e) =>
                setPrescription({ ...prescription, symtoms: e.target.value })
              }
              rows={4}
...
<button
  className="flex items-center justify-center px-6 py-2 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold rounded-lg shadow-md hover:from-blue-600 hover:to-blue-800 hover:shadow-lg transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 disabled:opacity-50"
  type="button"
